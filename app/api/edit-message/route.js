...
export async function POST(req) {
  try {
    const { roomId, messageId, newMessage, nickname } = await req.json();
    ...
    const messageIndex = room.messages.findIndex(msg => msg.messageId === messageId);
    if (messageIndex === -1) {
    ...
    room.messages[messageIndex].editedBy = nickname;
